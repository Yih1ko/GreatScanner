...
//********************************************** 外部接口 ****************************************
//设置初始角度(弃)，顺时针逆时针
void RoundProgressBar::setdefault(bool clockWise)
{
    //history:{参数:setdefault(int startAngle,bool clockWise)
    // _startAngle = startAngle;
    //}
    _startAngle = 0; // 固定在右侧（0度）
    _clockWise = clockWise; // 根据参数设置顺时针或逆时针
}
...
void RoundProgressBar::paintInnerBar(QPainter& painter)
{
    ...
    // 根据顺时针或逆时针方向设置起始角度和跨度
    if(_clockWise){
        int startAngle = 180 * 16;
        int spanAngle = (-180.0 * (_value - _min) / (_max - _min)) * 16;
...
void RoundProgressBar::paintDot(QPainter& painter)
{
    ...
    // 计算圆心坐标
    float centerX = _squareStart + _squareWidth / 2;
    float centerY = _squareStart + _squareWidth / 2;
    float radius = _squareWidth / 2;

    // 根据顺时针或逆时针方向设置装饰点位置
    if (_clockWise) {
        // 顺时针：最左边端点
        float dotX = centerX - radius;
        float dotY = centerY;
        painter.drawEllipse(dotX - _innerBarWidth/6, dotY - _innerBarWidth/6, _innerBarWidth/3, _innerBarWidth/3);
    } else {
        // 逆时针：最右边端点
        float dotX = centerX + radius;
        float dotY = centerY;
...
void RoundProgressBar::paintText(QPainter& painter)
{
    ...
    case value:
        painter.drawText(_squareStart,_squareStart-12,_squareWidth,_squareWidth,Qt::AlignCenter,QString::number(_value,'f',_precision));
        break;
    case valueAndMax:
        painter.drawText(_squareStart,_squareStart-12,_squareWidth,_squareWidth,Qt::AlignCenter,
