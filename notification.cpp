    ...
    m_bAppearEnd(false)
{
    ...
    if(type != NotifyType::Notify_Type_None)
    {
        ...
        {
            pIcon->setPixmap(QPixmap(":/icons/成功.png"));
        }
        else if(type == NotifyType::Notify_Type_Error)
        {
            pIcon->setPixmap(QPixmap(":/icons/错误.png"));
        }
        else if(type == NotifyType::Notify_Type_Warning)
        {
            pIcon->setPixmap(QPixmap(":/icons/告警实心.png"));
        }
        else
        {
            pIcon->setPixmap(QPixmap(":/icons/信息.png"));
        }
        pIcon->adjustSize();
...
void NotificationItem::Appear()
{
    ...
    // 添加缓动效果
    QEasingCurve curve;
    curve.setType(QEasingCurve::/*OutElastic*/OutCubic);
...
void NotificationItem::Disappear()
{
    ...
    // 添加缓动效果
    QEasingCurve curve;
    curve.setType(QEasingCurve::OutQuad);   // 示例：使用二次缓出曲线
    pOpacityAnimation2->setEasingCurve(curve);
...
void NotificationItem::paintEvent(QPaintEvent *event)
{
    ...
    // int shadowBorderWidth = 5;     // 阴影宽度
    // int borderRadius = 8;          // 圆角半径
    // drawEffectShadow(&p, widgetRect, shadowBorderWidth, borderRadius);
